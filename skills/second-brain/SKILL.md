...
- **setup** — conversational onboarding. Walk the user through this checklist; the engines do the work, you relay results and ask before each step. Reach for it on first run, when notes land in the vault root, or after adding a repo. Safe to repeat — everything merges.
  1. Run `scripts/doctor.py` and relay the report; offer `--fix` on failures.
  2. Ask which repos/projects memory should track, then edit the vault's `config.json` `project_map` directly (repo dir name → vault folder; map two repos to one folder to share memory — shape at the top of this file / `config.example.json`).
  3. If `scripts/starter-pack.py` exists, offer it: `--list` to show tiers, then `--tiers core|core,writing|core,writing,design`.
  4. Offer to connect Claude Desktop + Cursor: `python3 <mcp_dir>/mcp-setup.py --write` (`<mcp_dir>` = `${CLAUDE_PLUGIN_ROOT}/mcp` for the plugin, else `~/.claude/mcp`). Idempotent, backs up first.
  5. Mention the **experimental** ChatGPT/claude.ai remote (read-only server behind a cloudflared tunnel — see `mcp/README.md`). Opt-in only; never launch the tunnel yourself.
  6. Run `scripts/setup.py --verify-capture` and relay the ✓/✗ — it pipes a synthetic Stop payload through `capture-exchange.py` and checks today's `Daily/` note grew.
  7. Tell the user to restart Claude Code so the hooks load.
  (`scripts/setup.py` run bare is the same flow as a TTY wizard — use it when the user prefers a terminal.)
- **starter-pack `[--list] [--tiers core,writing,design]`** — run `scripts/starter-pack.py`: install the optional pack of skills, vault notes (`_playbook/`, `_templates/`) and an `.obsidian/` app config, then refresh `_system/`. `--list` first: it prints the tiers plus the third-party packs those skills route into (GStack, superpowers, ui-ux-pro-max, stop-slop…), which are fetched from their authors, never bundled. Nothing already on disk is overwritten, so it is safe to re-run. Offered as step 3 of `setup`.
