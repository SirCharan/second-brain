...
- **setup** — run `scripts/setup.py`: the guided first-run. Scans the usual code roots for git repositories, writes a real `project_map` into the vault's `config.json` so captures file themselves under the right project, optionally installs semantic recall, then runs `doctor`. Safe to re-run — it merges into the existing config. Reach for it when notes are landing in the vault root, or after adding a new repo.
